...
class TcSellsyConnector(TcSellsyConnectorBase):
    def __init__(self, conf: dict, logger, emailTemplates=None):
        super().__init__(conf, logger, emailTemplates)
        self.url = "https://apifeed.sellsy.com/0/"

    def _getConnector(self) -> None:
        self._client = requests_oauthlib.OAuth1Session(
            self.conf["consumer_token"],
            self.conf["consumer_secret"],
            self.conf["user_token"],
            self.conf["user_secret"],
    ...
    def _api(self, method="Infos.getInfos", params={}) -> dict:
        ...
        self.logger.debug(f"Sending POST to {self.url}")
