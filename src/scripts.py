    ...
    for _, client in clients.items():
        if client.telecommownAbo is not None and client.telecommownAbo:
            ...
            if opp is not None:
                ...
                sellsyConnector.updateCustomField(
                    "opportunity",
                    opp.id,
                    sellsyConnector.cfIdTeleCommownDateDebut,
                    datetime(year=2022, month=1, day=1).timestamp(),
                )
...
def createCustomField(sellsyConnector):
    data = {
        ...
        "Partenariat & Média": ["Partenariat & Média", "autre"],
        "RH": ["candidature", "autre"],
        "Souscription": [
            "pack dépannage",
            "pb code RIO",
    ...
    for name, fields in data.items():
        ...
        for f in fields:
            params["preferenceslist"].append({"value": f})
