...
class TestSelly:
    def test_get_opportunity(self, test_connector):
        ...
        test_connector.updateCustomField(
            "opportunity", id, cf["achatsimphysique"], True
        )
        test_connector.updateCustomField(
        ...
        test_connector.updateCustomField(
            "opportunity", id, cf["code-promo"], "TESTCODE"
        )
        test_connector.updateCustomField(
        ...
        test_connector.updateCustomField(
            "opportunity", id, cf["code-promo"], "TESTCODE2"
        )
        test_connector.updateCustomField(
    ...
    def test_get_client(self, test_connector):
        id = clientIdPG
        cf = sellsy.sellsyValues["DEV"]["custom_fields"]
        d = datetime.now()
        now = parisTZ.localize(datetime(year=d.year, month=d.month, day=d.day))
        ...
        test_connector.updateCustomField(
            "client", id, cf["parrainage-nb-code-donated"], 1
        )
        test_connector.updateCustomField(
            "client", id, cf["offre-telecommown"], timestamp
        )
        test_connector.updateCustomField(
        ...
        test_connector.updateCustomField(
            "client", id, cf["parrainage-nb-code-donated"], 2
        )
        ...
        test_connector.updateCustomField(
            "client", id, cf["telecommown-origine"], "Commown"
        )
    ...
    def test_get_client_opportunities(self, test_connector):
    ...
    def test_get_opportunity_client(self, test_connector):
